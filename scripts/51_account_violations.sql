...
-- (Moshe, SMS, Chat) or a plan resource-cap creation (members, cells,
-- branches) — written by src/lib/plan-gate.ts on every 403 it returns.
-- src/lib/health-checks.ts reads this to flag a church with an unusual
-- volume of blocked attempts as a possible-bypass system_alert, surfaced
-- on the existing Health & Alerts admin tab (no new UI needed).
-- ============================================================
...
CREATE TABLE IF NOT EXISTS access_violations (
  ...
  gate TEXT NOT NULL, -- 'premium' | 'chat' | 'resource_limit'
  created_at TIMESTAMPTZ NOT NULL DEFAULT NOW()
);
