...
export default function ThemeToggle({ dark, setDark, border }: { dark: boolean; setDark: (v: boolean | ((prev: boolean) => boolean)) => void; border: string }) {
  return (
    <div onClick={() => setDark(v => !v)} role="switch" aria-checked={dark} style={{ width: 50, height: 28, borderRadius: 14, border: `0.5px solid ${border}`, background: dark ? 'linear-gradient(135deg,#3C3489,#534AB7)' : '#EEEDFE', display: 'flex', alignItems: 'center', padding: 2, cursor: 'pointer', position: 'relative', flexShrink: 0, transition: 'background 0.25s ease' }}>
