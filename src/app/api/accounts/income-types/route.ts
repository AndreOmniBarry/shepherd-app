...
export async function POST(req: Request) {
  ...
  const data = await res.json();
  // name is NOT NULL on income_types and was never validated or checked
  // for a failed insert here (same class of bug found repeatedly this
  // session elsewhere).
  if (!res.ok) {
    console.error('[POST /api/accounts/income-types] insert failed:', data);
    return NextResponse.json({ data: null, error: { message: 'Failed to create income type' } }, { status: 500 });
  }
