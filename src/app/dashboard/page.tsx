      ...
      {activeTab === 'services' && (
        <div style={cardS()}>
          ...
          <div style={{fontSize:12,color:t.muted,marginBottom:14}}>
            How many main services in a row a member has to miss before they&apos;re escalated to your care team&apos;s queue for follow-up. A check runs automatically the day after your main service, whichever day of the week that is.
          </div>
          <div style={{display:'flex',alignItems:'center',gap:10}}>
            <select value={absenceThreshold} onChange={e=>setAbsenceThreshold(Number(e.target.value))} style={{...inputS,width:'auto',minWidth:160}}>
              {[1,2,3,4,5,6].map(n=><option key={n} value={n}>{n} missed service{n>1?'s':''} in a row</option>)}
            </select>
          </div>
          <div style={{marginTop:12,background:t.purpleBg,borderRadius:8,padding:'10px 14px',fontSize:11,color:t.purple}}>
            {absenceThreshold===1
              ? 'A care lead is created the very first time a member misses your main service.'
              : `A care lead is only created once a member has missed your main service ${absenceThreshold} times in a row — a single missed week alone won't trigger anything.`}
          </div>
        </div>
      )}
      {activeTab === 'services' && (
