          ...
          <div>
            ...
            <div style={{ ...card({ padding: 0 }), overflow: 'hidden' }}>
              ...
              {filteredMembers.length === 0 && (
                <div style={{ padding: 32, textAlign: 'center', color: t.muted, fontSize: 13 }}>No members found. Use the "Add member" button above to add your first member.</div>
              )}
            </div>
          </div>
        )}
...
function FellowshipAddMemberForm({ t, onClose, onSuccess }: { t: Record<string,string>; onClose: ()=>void; onSuccess: (msg:string)=>void }) {
  ...
  async function submit() {
    ...
    try {
      ...
      if (res.ok) {
        onSuccess('Member submitted. Once approved they will appear in your fellowship roster.');
      } else {
  ...
  const inputS: React.CSSProperties = { width: '100%', border: `0.5px solid ${t.border}`, borderRadius: 8, padding: '9px 11px', fontSize: 13, background: t.bg || '#F7F6FF', color: t.text, outline: 'none', fontFamily: 'inherit', boxSizing: 'border-box' as const };

  return (
    <div style={{ background: t.purpleBg, border: `0.5px solid rgba(83,74,183,0.2)`, borderRadius: 10, padding: '16px', marginBottom: 14 }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10, marginBottom: 10 }}>
        ...
        <div>
          ...
          <input type="date" value={form.date_of_birth} onChange={e => setForm(p=>({...p, date_of_birth: e.target.value}))} style={inputS} />
        </div>
        <div style={{ gridColumn: '1 / -1' }}>
          <div style={{ fontSize: 10, color: t.muted, textTransform: 'uppercase' as const, letterSpacing: '0.4px', marginBottom: 4 }}>Email</div>
          <input type="email" value={form.email} onChange={e => setForm(p=>({...p, email: e.target.value}))} placeholder="optional" style={inputS} />
        </div>
      </div>
      <div style={{ display: 'flex', gap: 8 }}>
