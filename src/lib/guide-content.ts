...
// The tier-1/tier-2 terminology a church actually uses (Fellowship/Cell,
// Zone/District, Campus/Fellowship, Department/Unit, Network/Home Group,
// or none at all for a single-congregation church) — resolved once from
// the church's own config and threaded through every guide string below,
// so the tour and Help panel never say "Cell"/"Fellowship" to a church
// that picked a different structure. Defaults match the words this
// content used to hardcode, so a church that never touches its config
// (or the pre-structure-agnostic default, cell_church) sees no change.
...
export function guideLabelsFromConfig(config?: {
  ...
  return {
    tier1: config?.tier1_label || 'Fellowship',
    tier2: config?.tier2_label || 'Cell',
    tier1Head: config?.tier1_head_label || 'Fellowship Head',
    tier2Head: config?.tier2_head_label || 'Cell Leader',
  };
}

// Minimal English pluraliser — mirrors src/lib/church-config.ts's
// pluralizeLabel but kept local so this file has no runtime dependency
// beyond the MockupKind type (church-config.ts is a much heavier module
// pulled into pages that don't otherwise need it).
function plural(label: string): string {
  return /[sxz]$|[cs]h$/i.test(label) ? `${label}es` : `${label}s`;
}

export function getPortalTitle(key: PortalKey, labels: GuideLabels): string {
  switch (key) {
    case 'dashboard': return 'Your Dashboard';
    case 'cell': return `Your ${labels.tier2} Portal`;
    case 'fellowship': return `Your ${labels.tier1} Portal`;
    case 'department': return 'Your Department Portal';
    case 'care': return 'Care & Follow-Up';
    case 'accounts': return 'Accounts Portal';
    case 'partnership': return 'Partnership Portal';
    case 'workforce': return 'Workforce Portal';
  }
}

// Kept as a static export too, for any caller that only ever runs for the
// cell_church defaults (none currently) — GuideHelpPanel and GuideTour
// both call getPortalTitle()/getGuideContent() with the church's real
// labels instead.
...
export function getGuideContent(labels: GuideLabels): Record<PortalKey, GuideEntry[]> {
  const { tier1, tier2, tier1Head } = labels;
  const tier1Lc = tier1.toLowerCase();
  const tier2Lc = tier2.toLowerCase();
  const tier2Pl = plural(tier2);
  const tier2PlLc = tier2Pl.toLowerCase();
  const tier1Pl = plural(tier1);
  ...
  return {
    dashboard: [
      ...
      { id: 'action-board', icon: 'ti-alert-triangle', title: 'Action Board', blurb: 'Flags what needs a decision now, before it becomes a bigger problem.', mockup: 'board', mockupLabels: ['Needs attention', 'On track'],
        steps: ['Tap Action Board in the menu.', `Review anything flagged — a quiet ${tier2Lc}, a short-staffed department, a member drifting from attendance.`, 'Tap an item to open it and take action directly.'] },
      ...
      { id: 'attendance', icon: 'ti-calendar-check', title: 'Attendance', blurb: `Church-wide attendance trends, without digging through individual ${tier2PlLc}.`, mockup: 'chart',
        steps: ['Tap Attendance in the menu.', 'View overall attendance trends and rates across every group.', `Use this for oversight — individual ${tier2PlLc} submit their own attendance from their own portal.`] },
      { id: 'structure', icon: 'ti-circles', title: 'Church Structure', blurb: 'How your church is organized, and how each group is performing.', mockup: 'tree', mockupLabels: [tier1, tier2, 'Member'],
        steps: [`Tap the ${tier1Lc} section in the menu.`, `See every ${tier1Lc} with its ${tier1Head.toLowerCase()}, size, and a letter grade for consistency.`, `Tap a ${tier1Lc} to see its own ${tier2PlLc} and attendance history.`] },
      { id: 'validate', icon: 'ti-checkbox', title: 'Validate Records', blurb: 'Confirm new members and record changes before they\'re treated as final.', mockup: 'form', mockupLabels: ['New member', `${tier2} change`],
      ...
      { id: 'settings', icon: 'ti-settings', title: 'Church Settings', blurb: 'Your church\'s name, structure, and the words the app uses for each level.', mockup: 'tree', mockupLabels: [tier1, tier2, 'Member'],
    ...
    cell: [
      { id: 'attendance', icon: 'ti-calendar-check', title: 'Attendance', blurb: 'The task you\'ll use every week — mark who was present.', mockup: 'form', mockupLabels: ['Service date', 'Mark present / absent'],
        steps: ['Tap Attendance in the menu.', 'Choose the service date.', `Mark each member Present or Absent, and add visitor count.`, `Tap Submit — you'll see your ${tier2Lc}'s attendance rate immediately.`] },
      { id: 'members', icon: 'ti-users', title: 'Members', blurb: `Everyone in your ${tier2Lc}, with their contact details.`, mockup: 'split',
      ...
      { id: 'meetings', icon: 'ti-calendar-event', title: `${tier2} Meetings`, blurb: 'Log what happened at your last meeting, beyond attendance numbers.', mockup: 'form', mockupLabels: ['What we covered'],
        steps: ['Tap Meetings in the menu.', 'Add a short note on your last gathering.'] },
      { id: 'history', icon: 'ti-chart-bar', title: 'Attendance History', blurb: `How your ${tier2Lc} has trended over the past several weeks.`, mockup: 'chart',
      ...
      { id: 'chat', icon: 'ti-message-circle', title: 'Chat', blurb: `Message your ${tier1Head} or your members directly.`, mockup: 'chat',
        steps: ['Tap the chat icon at the top.', 'Choose a conversation or start a new one.'] },
    ],
    fellowship: [
      { id: 'overview', icon: 'ti-circles', title: `${tier2Pl} Overview`, blurb: `Every ${tier2Lc} under your ${tier1}, and how each is doing.`, mockup: 'list', mockupLabels: [`Overcomers ${tier2}`, `Grace ${tier2}`],
        steps: ['Tap Overview in the menu.', `See each ${tier2Lc} with its leader, size, and grade.`, `Tap a ${tier2Lc} for its full detail and history.`] },
      { id: 'members', icon: 'ti-users', title: 'Members', blurb: `Everyone across your whole ${tier1} in one list.`, mockup: 'split',
        steps: ['Tap Members in the menu.', `Search or filter by ${tier2Lc}.`, `Move a member between ${tier2PlLc} directly from their row.`] },
      { id: 'cells-manage', icon: 'ti-building', title: `Managing ${tier2Pl}`, blurb: `Rename a ${tier2Lc}, or merge two small ${tier2PlLc} into one.`, mockup: 'board', mockupLabels: ['Keep', 'Merge in'],
        steps: [`Open a ${tier2Lc} and tap its name to rename it.`, `Use "Create a new ${tier2Lc}" to add one.`, `Use "Collapse ${tier2PlLc}" to merge smaller ${tier2PlLc} together.`] },
      { id: 'history', icon: 'ti-chart-bar', title: 'Attendance & SLA', blurb: `A grade for each ${tier2Lc} showing how consistently it's reporting.`, mockup: 'chart',
        steps: [`Open any ${tier2Lc} to see its attendance trend and current grade.`, `Use this to spot which ${tier2PlLc} need support.`] },
      { id: 'prayer', icon: 'ti-heart', title: 'Prayer Requests', blurb: `Requests submitted across your ${tier1}.`, mockup: 'list', mockupLabels: ['Healing for a family member', 'New job'],
...
// Kept as a static export too, for symmetry with PORTAL_TITLE above — no
// current caller uses it; GuideHelpPanel and GuideTour call
// getGuideContent(labels) with the church's real labels instead.
export const GUIDE_CONTENT: Record<PortalKey, GuideEntry[]> = getGuideContent(guideLabelsFromConfig(null));
